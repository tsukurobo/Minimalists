...
trajectory_t::trajectory_t(float max_vel, float max_accel, float start_pos, float end_pos)
    : max_vel_(max_vel),
...
void trajectory_t::calculate_trapezoidal_params() {
    ...
    float accel_dist = 0.5 * max_accel_ * accel_time_ * accel_time_;
...
void trajectory_t::get_trapezoidal_state(float current_time, float *target_pos, float *target_vel, float *target_accel) const {
    float direction = (end_pos_ > start_pos_) ? 1.0 : -1.0;
