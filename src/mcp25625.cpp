...
bool mcp25625_t::send_can_message(const struct can_frame_t* frame) {
    ...
    // 送信バッファが空くまでポーリング（TXREQ=0を待つ） [cite: 487]
    ...
    return true;  // 送信要求が成功した

    // // 送信完了までポーリング (TXREQビットが0になるのを待つ)
    // constexpr int max_wait = 1000;  // 最大ループ回数（タイムアウト防止用）
    // for (int i = 0; i < max_wait; ++i) {
    //     status = _read_register(MCP_TXB0CTRL);
    //     if ((status & 0x08) == 0) {
    //         return true;  // 送信完了
    //     }
    //     sleep_us(1);  // 必要に応じて調整（例: 100us）
    // }
